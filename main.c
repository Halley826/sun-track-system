...
void USART1_IRQHandler() // GPRS串口中断服务程序
{
	...
	if (USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)
	{
		...
		if (USART_GetITStatus(USART1, USART_IT_RXNE) == SET) //接受指令
		{
			if (t == 5)
			{
				*p = USART_ReceiveData(USART1);
				...
				else if (strncmp(arrwc, received, 6) == 0) //大风关闭
				{
					Gstate = 0;
				...
				else if (strncmp(arrsc, received, 6) == 0)
				{
					Gstate = 0;
				...
				else if (strncmp(arro, received, 6) == 0)
